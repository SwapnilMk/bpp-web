...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Anchor
    ...
    ref={ref}
    className={cn(
      'shadow-xs data-focused:ring-1 data-focused:ring-ring relative flex h-9 w-full items-center justify-between gap-2 rounded-md border border-input bg-transparent px-3 py-2',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Input
    ...
    ref={ref}
    className={cn(
      'focus:outline-hidden flex h-9 w-full rounded-md bg-transparent text-base placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <ComboboxPrimitive.Trigger
    ...
    ref={ref}
    className={cn(
      'focus-visible:outline-hidden flex shrink-0 items-center justify-center rounded-r-md border-input bg-transparent text-muted-foreground transition-colors hover:text-foreground/80 disabled:cursor-not-allowed disabled:opacity-50',
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  ...
  >
    ...
    <ComboboxPrimitive.BadgeItemDelete
      ...
      className='focus-visible:outline-hidden data-highlighted:bg-destructive shrink-0 rounded p-0.5 opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus-visible:ring-2 focus-visible:ring-ring'
    >
      <X className='h-3 w-3' />
    </ComboboxPrimitive.BadgeItemDelete>
  </ComboboxPrimitive.BadgeItem>
...
>(({ sideOffset = 6, className, children, ...props }, ref) => (
  <ComboboxPrimitive.Portal>
    <ComboboxPrimitive.Content
      ...
      className={cn(
        'relative z-50 max-h-fit min-w-[var(--dice-anchor-width)] origin-[var(--dice-transform-origin)] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        className
      )}
      {...props}
    >
...
>(({ className, children, outset, ...props }, ref) => (
  <ComboboxPrimitive.Item
    ...
    ref={ref}
    className={cn(
      'outline-hidden data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-50 relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 text-sm',
