    ...
    ({ className, children, ...props }, ref) => {
        return (
            <div className="relative">
                ...
                {!props.multiple && (
                    <span className="flex absolute inset-y-0 justify-center items-center w-9 h-full pointer-events-none end-0 text-muted-foreground/80 peer-disabled:opacity-50">
                        <ChevronDownIcon className="w-4 h-4" strokeWidth={2} aria-hidden="true" />
                    </span>
                )}
            </div>
        );
